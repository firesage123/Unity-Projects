...
public class TankControl : MonoBehaviour {
    ...
    void Update() {
        if (DeadZone(Input.GetAxis(VerticalAxis)) != 0) {
            float sDesired = Input.GetAxis(VerticalAxis) * ForwardSpeed;
            
            float sActual = Vector3.Project(tankRb.velocity, transform.forward).magnitude;
            
            float force = Acceleration * (sDesired - sActual);    
            float forceR = -1*Vector3.Project(tankRb.velocity, transform.right).magnitude;
            
            //tankRb.velocity += (force*(Vector2)transform.up + forceR*(Vector2)transform.right)*Time.deltaTime;
            
            tankRb.AddForce(force*transform.up);
            tankRb.AddForce(forceR * transform.right);        
        }
        if (DeadZone(Input.GetAxis(HorizontalAxis)) != 0) {
            tankRb.rotation += Input.GetAxis(HorizontalAxis) * RotationSpeed * Time.deltaTime;
        }
