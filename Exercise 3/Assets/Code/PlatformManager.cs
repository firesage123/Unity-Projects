...
public class PlatformManager : MonoBehaviour {
	...
	internal void Update () {
		if (playerNotDead == true) {
			if (RecycleDistance < transform.position.x - OldestPlatformInUse.transform.position.x)
				RecycleOldestPlatform ();
			if (nextSpawnPoint.x  - transform.position.x < SpawnAtDistance)
				SpawnNewPlatform ();
		}
	}
	internal void RecycleOldestPlatform () {
		var pltfrm = platformsInUse.Dequeue ();
		unusedPlatforms.Enqueue (pltfrm);
		pltfrm.SetActive (false);
	}

	public void MovePlatformToSpawnPoint (GameObject platform) {
